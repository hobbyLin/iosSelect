  ...
  export default{
    ...
    methods: {
      ...
      setWheelItemDeg (index) {
          // 兼容性添加
        return {
          webkitTransform: `rotate3d(1, 0, 0, ${-index * 19 % 360}deg) translate3d(0px, 0px, 100px)`,
          transform: `rotate3d(1, 0, 0, ${-index * 19 % 360}deg) translate3d(0px, 0px, 100px)`,
      ...
      setWheelDeg (updateDeg, type, time = 1000) {
        if (type === 'end') {
          ...
          this.$refs.wheel.style.transition = `transform ${time}ms cubic-bezier(0.19, 1, 0.22, 1)`
          this.$refs.wheel.style.transform = `rotate3d(1, 0, 0, ${updateDeg}deg)`
      ...
      setListTransform (translateY = 0, marginTop = 0, type, time = 1000) {
        if (type === 'end') {
          ...
          this.$refs.list.style.transform = `translateY(${translateY - this.spin.branch * 34}px)`
      ...
      setStyle (move, type, time) {
        ...
        let spinAim = Math.round(updateDeg / deg)
